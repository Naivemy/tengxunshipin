...
let naviBarItems = naviBar.querySelectorAll("li>a");
let pages = document.querySelectorAll("#pages>.page");
for (let j = 0; j < pages.length; j++) {
    pages[j].style.display = "none";
}
pages[0].style.display = "block";
naviBarItems[0].classList.add("active");

for (let i = 0; i < naviBarItems.length; i++) {
    let item = naviBarItems[i];
    item.addEventListener("click", function(e) {
        for (let j = 0; j < pages.length; j++) {
            pages[j].style.display = "none";
        }
        for (let j = 0; j < naviBarItems.length; j++) {
            naviBarItems[j].classList.remove("active");
        }
        pages[i].style.display = "block";
        naviBarItems[i].classList.add("active");
        e.stopPropagation();
    });
}